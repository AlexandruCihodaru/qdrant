...
pub async fn do_delete_full_snapshot(
    ...
    let storage_config = dispatcher
        .clone()
        .toc()
        .storage_config
        .to_shared_storage_config(false);
    let snapshot_dir = get_full_snapshot_path(dispatcher.toc(), snapshot_name).await?;
    let snapshot_manager = storage_config.snapshot_manager();
...
async fn _do_create_full_snapshot(
    ...
    let snapshot_manager = dispatcher
        .toc()
        .storage_config
        .to_shared_storage_config(false)
        .snapshot_manager();
